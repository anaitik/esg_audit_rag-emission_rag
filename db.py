...
def create_iro(
    ...
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO iros (framework_id, topic, sub_topic, materiality_scope, stakeholder_groups, description)
           VALUES (?, ?, ?, ?, ?, ?)""",
        (framework_id, topic, sub_topic, materiality_scope, stakeholder_groups, description)
    )
    iid = cur.lastrowid
    conn.commit()
    conn.close()
...
def list_iros(framework_id: Optional[int] = None) -> List[dict]:
    ...
    if framework_id is not None:
        cur.execute(
            "SELECT id, framework_id, topic, sub_topic, materiality_scope, stakeholder_groups, description, created_at FROM iros WHERE framework_id = ? ORDER BY created_at DESC",
            (framework_id,)
        )
    else:
        cur.execute(
            "SELECT id, framework_id, topic, sub_topic, materiality_scope, stakeholder_groups, description, created_at FROM iros ORDER BY created_at DESC"
        )
    rows = cur.fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def add_evidence_file(evidence_tag: str, file_path: str, original_name: str) -> int:
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO evidence_files (evidence_tag, file_path, original_name) VALUES (?, ?, ?)",
        (evidence_tag, file_path, original_name)
    )
    eid = cur.lastrowid
    conn.commit()
    conn.close()
...
def save_report_value(
    ...
    cur.execute(
        """INSERT INTO report_values (evidence_tag, metric_name, calculated_value, unit, calculator_module_id, evidence_file_ids)
           VALUES (?, ?, ?, ?, ?, ?)""",
        (evidence_tag, metric_name, str(calculated_value), unit, calculator_module_id, eids_str)
    )
    rid = cur.lastrowid
    conn.commit()
    conn.close()
...
def list_report_values(evidence_tag: Optional[str] = None) -> List[dict]:
    conn = get_connection()
    cur = conn.cursor()
    if evidence_tag:
        cur.execute(
            """SELECT r.id, r.evidence_tag, r.metric_name, r.calculated_value, r.unit, r.calculator_module_id, r.evidence_file_ids, r.computed_at
